...
# 判断输入模块，输入的不是数字就重新进入循环
def numif(number_input):
    while not number_input.isdigit():  # 输入不是数字就进入循环
        number_input = input('\033[31m 输入%s不是数字，请重新输入!\033[0m' % number_input)
    number = number_input
    return number
...
def revise(revise_input):  # 定义修改功能
    ...
    if revise_input in backend_show_dict:  # 判断输入是否存在
        ...
        if revise_choise == 'y':
            ...
            backend_all_dict[backend_revise_new] = backend_all_dict.pop(
                backend_revise)  # 将旧backend的server对应到修改后的backend上
            ...
            if revise_choise_server == 'y':  # 继续修改server
                ...
                server_revise_dict_backup[
                    backend_revise_new] = server_revise_dict.values()  # 将修改的server信息对应到修改后的backend存到字典中
